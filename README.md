...
/exec  Three-agent isolated execution
  ...
  │
  ▼
/test-webview  web-interaction validation (frontend; optional)   /test-api  backend API e2e (Go backend; optional)
  │  brings the app up via the project's Docker stack              │  brings the stack up via docker-compose.e2e.yaml
  │  (confirmed once, remembered) → cleans up after                │  (testapi build tag) → cleans up after
  │  each case runs in an isolated serial subagent over            │  md cases compile to Go test → one go test run finishes all
...
- **`/test-api`** — **backend API e2e** (Go backend projects). Brings the stack up via the project-owned `docker-compose.e2e.yaml` (backend image **must** be built with the `testapi` build tag), then **mechanically compiles md cases under `.bb-spec/docs/test/api/` into Go test files** and runs them in one shot via `go test -tags testapi -json -count=1 ./...` — **zero subagents, zero concurrency** (HTTP is a deterministic script, no need for an LLM to step through; clock state is shared, so concurrency is banned). **Time-sensitive rules** (token expiry, order timeout, points expiry) are tested through build-tag-isolated `/test/advance-time`, `/test/backdate`, `/test/trigger-job` endpoints — the application side adopts `clockwork` clock injection plus a `testapi` build tag that **compile-time isolates `/test/*` routes from the production build**; a failed `/test/healthz` probe aborts the run, no fallback. Cases are generated by `/test-api` itself from spec / plan / PRD; a full run first **aligns coverage** and lists uncovered API scenarios (gaps are never silently dropped); failures route to `/revise`. Generated Go test files land in `.bb-spec/.cache/test-api-gen/` — the md docs are the single source of truth.
