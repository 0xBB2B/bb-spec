...
/exec  Three-agent isolated execution
  ...
  │
  ▼
/test-webview  web-interaction validation (frontend projects; optional)
  │  brings the app up via the project's Docker stack (confirmed once, remembered) → cleans up after
  │  each case runs in an isolated serial subagent over the browser MCP (no context blow-up)
  │  failures route to /revise
  │
  ▼
