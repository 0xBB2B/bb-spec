...
需求清单之外的改动一律先经用户批准。唯一例外是安全性 / 鲁棒性 / 性能修复：可直接改，但同样遵守 R3.1 最小改动、不借机重构，且须在产出报告中逐条列出。
...
发现想写这类表述 → 直接彻底替换，不留过渡。

### R4.2 清除过期内容（按范围分流）

过期、无效的代码 / 文档 / 注释必须清除，按范围分流：
- **本次改动导致过期的**：直接清。
- **本次范围外发现的**（死代码、过时实现、过期文档 / 注释）：不动手，列清单写入产出报告，经用户批准后再清。
