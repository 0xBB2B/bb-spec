...
每份含 frontmatter + 正文六块（见下方模板）。**函数清单**：函数名 + 文件路径 + 职责一句话 + 调用关系 + 外部接口依赖；**不写**参数列表、返回值、实现；命名风格与项目一致。**成品定义**：涉及声明式产物（SQL DDL/API 契约/配置）时增设该块，直接给出可落盘的最终内容，exec 阶段原样使用、不得改写。
