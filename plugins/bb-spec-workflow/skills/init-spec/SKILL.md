---
name: init-spec
description: 项目初始化反向 spec 化（与 /spec 正向对称）——读既有代码与文档，把已在执行的隐式规范沉淀为 ≤100 行/份的 spec，落点与格式完全对齐 /spec；产出代码无关，读者不需打开本项目源码即可理解，能与 /spec 正向文档 round-trip 匹配；大项目按功能区拆 subagent 并发提取、主 agent 串行落盘。触发：/init-spec、给现有项目反向生成 spec、把现有规范文档化、项目接入 bb-spec 工作流。跳过：已有完整 spec 且无需补全、新项目无现存代码可提炼（应走 /prd→/spec）。
---
...
2. **闭环对齐 `/spec`**：`/spec → /plan → /exec` 产出的代码，用 `/init-spec` 反推回来的 spec 必须与原始 spec 基本一致——同格式、同粒度、同抽象层级。落盘的 spec **代码无关、项目无关**：读者不需要打开本项目源码就能理解每条规则；frontmatter、路径、`INDEX.md` 与 `/spec` 一致
3. **每条规则必举例，例子必须抽象**：例子写成"输入 · 过程 · 预期结果"的抽象场景，与 `/spec` 例子风格一致，脱离本项目代码仍成立
4. **`source_refs` 仅为提取阶段的存在性证据**：subagent 提交候选规则时须附 ≥1 处代码坐标，用于主 agent 判断"这条规则确实来自真实代码"、做筛选与冲突裁决；主 agent 落盘时按步骤 5 的模板生成 spec，`source_refs` 不出现在模板里
5. **并发只拆分区不拆决策**：subagent 只产**结构化候选规则草案**且**禁写盘**；所有 Write/Edit 由主 agent 串行执行以保证格式一致
6. **用户裁决拆分**：分区清单先给用户确认再派工，避免基于错误切分跑一长串无效 agent
7. **纯净现态**：只写**当前代码已在执行**的规则，禁写"应该但还没做"或"历史遗留"（用户想补"应然"则提示走 `/spec`）
8. **冲突不自决**：代码本身的冲突一律抛给用户裁决
9. **单文件 ≤ 100 行、禁跨文档引用、语言跟随用户**：超 100 行即拆；每份 spec 自包含不写"详见 X.md"；正文用用户的工作语言，标识符/API/错误码保持英文
...
## 例子
<一个具体场景：输入、过程、预期结果。与 `/spec` 例子风格一致。>
