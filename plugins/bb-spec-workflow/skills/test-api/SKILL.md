---
name: test-api
description: 后端 API e2e 验证——Docker 整栈拉起→md 用例编译为 Go test→一次性 `go test` 跑完→报告与 /revise 闭环；时间敏感规则（token 过期 / 订单超时 / 积分过期）经 build-tag 隔离的 /test/advance-time、/test/backdate、/test/trigger-job 测；零 subagent、零并发；跑完无条件 docker compose down -v。触发：/test-api、跑后端接口测试、API e2e 验收、验证后端业务流。跳过：非 Go 后端、Docker 不可用、应用未接入 testapi build tag。
argument-hint: [scope]
...
用项目自带 Docker 整栈拉起后端服务，把 md 用例**编译成 Go test 文件**后一次性 `go test` 跑完。核心：**API e2e 是确定性脚本，不派 subagent、不逐步操作**——agent 只做生成、调度、解析报告，真正执行由 `go test` 接管。
...
3. **缓存目录约定**：本 skill 临时产物（生成的 Go test、go test JSON 输出、docker logs）统一落 `${CACHE_DIR}/test-api-gen/`、`${CACHE_DIR}/test-api-logs/`，**首次写入前**确保 `${CACHE_DIR}/.gitignore` 存在——不存在则连目录一并创建、内容写单行 `*`。
4. `$ARGUMENTS` 非空 → 仅跑该 scope，否则全量。

### 步骤 1：前置检查（任一不过即中止并给指引）

- **是 Go 后端项目**：root 内存在 `go.mod`；多 repo 模式则要求至少一个后端 repo 含 `go.mod`。非 Go → 告知"本 skill 第一版仅支持 Go 后端"，中止。
- **Docker 可用**：`docker info` 探测；不可用 → 报错中止（提示安装 / 启动 Docker）。
- **Go 工具链可用**：`go version` 探测；不可用 → 报错中止。
...
${CACHE_DIR}/test-api-gen/
  ...
  go.sum
  helpers/
    client.go                  # HTTP 客户端 + action 映射（request/expect/extract/advanceTime/expireEntity/triggerJob/seed/cleanup）
    asserts.go                 # 基于 testify 的断言封装
  <scope>/<category>_test.go   # 由 md 用例机械渲染；包含 dependsOn 拓扑序的子测试
```

**依赖拓扑**：`dependsOn` 上游用例的 `*testing.T.Run` 子测试**先注册**；上游失败 → 下游用 `t.Skip` 跳过并级联（编译期按拓扑排序生成代码，运行时通过共享 map 记录上游 pass/fail 决策跳过）。依赖成环 → 编译阶段报错点名、不进入执行。

**执行**：
```bash
cd ${CACHE_DIR}/test-api-gen
go test -tags testapi -json -count=1 ./... > ${CACHE_DIR}/test-api-logs/result.json
```
（不传 `-parallel`、生成的代码不调 `t.Parallel()`——时钟串行约束）

### 步骤 5：聚合报告 + 回写状态 + 下一步

解析 `result.json`（`go test -json` 流式 JSON），汇总写报告，更新 `${DOCS_DIR}/test/api/INDEX.md` last-run 状态列。
...
无论通过、失败、中断，都执行：`docker compose -f <compose_file> down -v` 拆容器 + 删数据卷。生成的 Go test 文件保留在 `${CACHE_DIR}/test-api-gen/` 供二次调试（`.cache` 已被 gitignore，不污染版本控制）。
...
env:
  ...
  test_endpoints_prefix: /test                     # build-tag 注入的测试接口前缀
---
```

**用例 md 骨架、JSON 流字段、action 词表**：见 `references/api-testcase-format.md`。
**应用侧 Go 改造规范（clockwork + testapi build tag + /test/* 路由）**：见 `references/test-only-endpoints.md`。
**md → Go test 的代码生成模板**：见 `references/go-codegen-template.md`。
...
- Docker / Go 工具链 / `/test/healthz` 任一不可用即中止并给指引，**禁降级**为无环境跑
