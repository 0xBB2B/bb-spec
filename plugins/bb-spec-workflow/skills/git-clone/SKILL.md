...
**职责边界**：只解决「拿代码 + 写 base_dir」。spec 反向生成交给 `/init-spec`、技术栈识别交给各 constraints skill；本 skill 不触发它们、不读代码、不装依赖。
...
- 下一步建议：
  - cd <落点> 开始工作
  - 项目已有代码、想接入 bb-spec 工作流 → /init-spec 反向生成 spec
  - 新需求开工 → /spec
```
...
- 自动触发 `/init-spec` / 装依赖 / 跑测试——这些超出本 skill 职责
