...
简表之后立即用 AskUserQuestion 询问（不预先展开任何一项），选项：
  · **修复全部**（推荐）→ 对整张确认表一次性授权，按表顺序逐项展开并修复，中途不停等
  · **一个个开始修** → 从第 1 个问题起，逐个讲解、逐项确认后修
  · **批量清 NIT** → 一次处理 🟢 NIT 表
  （要从指定项开始讲解，用 Other 填项目编号，如 "3"；要看被否决的项回复「展开否决项」；不修回复「结束」）

确认队列清空后再问一次：**批量清 NIT** / **跳过 NIT，进入修复复审**；用户回复「结束」或队列与 NIT 都清空时进入修复复审。

### 逐个解决模式

用户选「修复全部」或「一个个开始修」后进入循环，**一轮只展开、只处理一个问题**，处理顺序 = 确认问题表顺序（质量/安全 → 风险 ✓ → 严重度 → ⭐）。两种模式只差在是否停等：

- **一个个开始修**：每项展开后结束回合等用户回应，用户对该项显式确认后才修
- **修复全部**：「修复全部」的选择即对整张确认表的一次性授权；每项仍按骨架完整展开（保证可追溯），展开后不结束回合、直接走 /revise，传参末尾注明「修复全部模式：用户已对本项授权，轻量修复判断满足条件时视为已同意」；revise 因方向分歧（冲突分析简报）停下时照常等用户裁决，裁决后继续；用户随时可打断
...
2. **对话解决**（一个个开始修）：展开当前问题后必须结束回合，停下等用户回应——确认修 / 讨论调整方案 / 跳过。「一个个开始修」与回复编号只是进入讲解流程，不构成任何一项的修复授权；每项的修复授权必须是用户看到该项完整展开后针对该项的显式回复。用户确认后调用 /revise 修复（见上方规则），一次只修当前这一类问题；revise 的完成简报即该项闭环。修复全部模式跳过本步的停等，其余规则不变。
...
- 逐个解决模式中，任何一项未完整展开前禁止调用 /revise 或改动任何文件；修复授权只能来自「修复全部」的一次性授权或「一个个开始修」下用户对该项的显式确认（文档同步类例外除外）
