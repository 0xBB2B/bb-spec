...
- 逐个解决模式展开该项时，解决方案统一写 `走 /revise 诊断（测试层 impl-defect）`，不直接给修测试的代码方案
- 若 finding 暗示 spec 对预期行为描述不清导致测试写错，解决方案写 `走 /revise 诊断（疑似 spec-defect）`
...
1. **展开当前问题**（仅此一条）：
   ...
   问题在哪（按 finding 性质二选一，不许只给抽象描述）：
     · 行为类（正确性 / 安全 / 性能）→ 编号触发步骤，从触发方（终端用户或调用方）
       视角写最小复现：1. 触发方传入 X → 2. 走到 file:42，此时变量 Y 为空 →
       3. 第 45 行解引用 → 4. 最终表现：panic / 越权 / 脏数据
     · 非行为类（可维护性 / 简洁性 / 文档同步）→ 代码证据 + 后果场景：摘录实际片段，
       说明"下次有人改 X 会因 Y 踩坑" / "文档说 A 代码做 B，照文档写会错"
   解决方案（只给一个，用对比表展示，禁贴代码片段对比）：先一句话说明改哪个
     文件哪几行，再用表格对照、一行一项、一目了然。表格行跟随"问题在哪"的性质：

     | 时刻 / 对比项 | 现状 | 修改后 |
     |---|---|---|
     | 行为类：触发步骤 1..n（每步一行） | 该步现行逻辑 / 走到的分支 | 该步新逻辑 / 走到的分支 |
     | 非行为类：对比项（代码证据 / 文档表述） | 现状内容 | 修改后内容 |

     完整代码改动等用户确认后动手时再写
   根源性自检（呈现方案前自问，结论对用户可见）：①该方案触及根源还是缓解症状？
     ②有无更优做法？自检发现更优 → 直接替换上面的方案再呈现（不追加备选）；
     确认已是根源解 → 一句话给出判断依据。注意：Verify 阶段的根源性仲裁裁决的是
     finder 的原始 suggestion，此处方案是展开时新写的，必须独立自检，不得以
     "对抗验证已通过"代替
   ```

2. **对话解决**：停下等用户回应——确认修 / 讨论调整方案 / 跳过。用户确认后才动手，修复遵循外科手术式改动，只修当前这一个问题。
3. **修复后对照确认**：修完立即给一张前后对照表请用户确认。表格的行直接复用"问题在哪"的内容——行为类沿原编号触发步骤把修复后的代码重走一遍，逐步对比；非行为类对照原代码证据与后果场景：
