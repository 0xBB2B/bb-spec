---
name: plan
description: 读 spec + 项目代码结构产出函数级实施计划——启动即进入只读对齐、方案获批后才落盘到 .bb-spec/docs/plan/<日期>.<主题>/；启动先 worktree 感知定位 plan 现场（主仓库停在 main 时自动定位到进行中的 worktree）；过程式只到函数级，声明式产物（DDL/API 契约/配置）必须内联完整成品。触发：/plan、生成实施计划、怎么落地这些 spec。跳过：还没 spec（→/spec）、spec 无变更。
---
...
1. **方案先对齐再落盘**：启动即进入只读对齐阶段（禁止任何写盘），方案呈现并获用户明确批准后才落盘（❌ 一进来就写文档）
...
9. **可断点恢复**：进度持久化到 `PROGRESS.md`，跨会话 / 新会话后从断点继续（❌ 执行时不读/不更新）
...
| `plan/<主题>/PROGRESS.md` | 执行断点唯一事实源，跨会话 / 新会话后据此恢复 | — |
...
**立即声明进入只读对齐阶段**（本阶段禁止任何写盘）。后续 0~3 步全在只读态完成。
...
- **非 git / 无 main / spec 目录不存在**：告知"建议先运行 `/spec`"，终止（不落盘）
- **diff 空**：告知"无 spec 变更，无需 plan"，终止（不落盘）
...
- **新增第三方依赖**（**单独成节，必列**）：每项写「库名 + 用途 + 版本策略」；版本策略写选版规则（如「最新稳定版」「最新 LTS」）而非具体版本号，具体版本号由 exec 落盘前经官方渠道查询；无新增则显式写「无」。**批准即授权**：用户批准本方案即视为对该清单的明确同意（version-policy 要求的新增第三方库同意流程在此一次完成），exec 阶段引入依赖不得超出该清单
- 同时盘点已有 plan（读 `${DOCS_DIR}/plan/INDEX.md`），指出冲突 / 可复用 / 需修订项

明确请求用户批准（等待用户答复，不得自行视为已批准）。批准后进入步骤 4；驳回则在只读态调整后重新呈现。
