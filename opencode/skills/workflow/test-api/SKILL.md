...
1. **零 subagent**：API 调用是确定性 HTTP，无 DOM 漂、无视觉判断，不需要 LLM 逐步介入。主 agent 一次渲染完整 runner → 一条命令跑完 → 解析 JSON 报告。**禁派任何 subagent**。
...
- **零 subagent、零并发**：编排不派 subagent，runner 内 `await` 顺序执行用例、**禁 `Promise.all` 等并发**
