---
name: doc-update
description: 全仓 spec/文档/代码一致性维护——扫描仓库内 spec、README、AGENTS.md/CLAUDE.md、plan 与代码，找出描述与实现的漂移；默认改 spec/文档让其追平代码现态，代码明显违背硬约束或存在不合理设计时停下用 `question` 工具 请用户裁决再改代码；落盘前汇总展示、过程中 INDEX 同步维护。触发：/doc-update、文档和代码不一致、spec 跟代码对不上、批量同步 spec 与现状、维护一下文档。跳过：还没有 spec 可对照（→/spec）、单点 bug 或定向修复（→/revise）、纯新增需求（→/spec→/plan）。
---

# Doc-Update 全仓文档一致性维护

把仓库内 **spec、README、AGENTS.md/CLAUDE.md、plan、代码内说明性文字** 跟代码现态对齐：默认认为「代码是事实」，spec/文档追平代码；**仅当代码明显违背硬约束或存在不合理设计**时停下来用 `question` 工具 请用户裁决，确认后才改代码。
...
2. **改码必须用户裁决**：本 skill 默认 **不动代码**；发现代码"明显不合理"才停下用 `question` 工具 请用户拍板，用户同意后转 `/revise` 走 TDD 流程，本 skill 不直接改实现
3. **批改前先汇总**：所有漂移分类列表 + 默认动作 + 待裁决项必须一次性展示，用户批准后才落盘，禁逐条边问边改打断节奏
4. **冲突不自决**：spec 内部互斥（两份 spec 互相打架）或同一 README 段落与多份 spec 冲突时，一律抛 `question` 工具，禁主 agent 私自选边
...
- **INDEX.md 不存在或为空** → 提示"项目尚未生成 spec，请先用 `/spec` 沉淀规则后再来 `/doc-update`"并退出
- **存在** → 进入步骤 1
...
2. **读项目根文档**：`README.md` / `README.zh.md` / `AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md` / `ARCHITECTURE.md` 等存在者
...
| B | **doc-stale** | README/AGENTS.md/CLAUDE.md/根文档引用了被改名、删除、行为变更的函数/路径/命令/配置项 | 改文档追平代码 |
| C | **code-violation** | 代码**明显**违背 spec 的硬约束（如错误码格式、命名约定、安全 invariant），且无合理理由 | **`question` 工具** → 用户同意改码 → 挂 `/revise` |
| D | **spec-conflict** | 两份及以上 spec 内部互斥 / 同一文档段落同时与多条 spec 冲突 | **`question` 工具** 让用户选保留哪份 |
...
—— 待裁决类别（C/D）逐条用 `question` 工具；A/B/E/F 默认动作请确认整体批准。
```

之后：

1. **C/D 类逐条 `question` 工具**：把"漂移点 / 候选方向 / 各方向代价"做成选项让用户选；C 类选项至少含「改代码（走 /revise）」「改 spec（认可代码现态）」「跳过本条」；D 类至少含「保留 spec X」「保留 spec Y」「合并为一条」「跳过」
2. **A/B/E/F 整体批准**：一次 `question` 工具 让用户选「全部批准 / 逐条裁决 / 取消」；"逐条裁决"模式下退化为对每条单独问
...
3. **改 README/AGENTS.md/CLAUDE.md**：精准替换被引用的旧 symbol / 路径 / 命令；语言保留与原文一致
4. **新增 spec（F 类）**：用与 `/spec` 一致的模板（`目的 / 逻辑 / 约束 / 例子 / 验收`），落 `${DOCS_DIR}/spec/<分区>/<name>.md`，例子末尾标注 `来源：file:line`，同步 INDEX
5. **C 类已批准改码项**：本 skill **不直接改实现**，而是产出一份"挂单清单"让用户复制给 `/revise`，每条按下方格式分行写：

   ```
   - [<分区>/<规则>] <涉及文件>
     期望行为：<spec 要求什么>
     不符点：<代码实际怎样>
     建议测试切入：<从哪个测试入口先写 Red>
   ```
...
3. **链接可达**：被改过的 README/AGENTS.md/CLAUDE.md 里引用的相对路径真实存在
...
- 已落盘：
  ...
  - INDEX 同步 Y 条
  - 根文档编辑 Z 处（README/AGENTS.md/CLAUDE.md/…）
  - 新增 spec K 条（F 类）
- 待用户后续处理：
...
- **上游**：`/spec`（沉淀规则）→ `/plan` + `/exec` 实施 → 长期演进后跑 `/doc-update` 做一致性体检
