...
- 依赖已列入**经用户批准的实施计划**的「新增第三方依赖」清单（如 `/plan` 的方案批准）→ 批准即同意，不需重复询问（版本仍按 §3 查最新）
...
**兜底**：以上命令不可用时，用 `webfetch` 工具或联网检索访问官方 release 页或 changelog；**不可**直接输出搜索结果摘要中的版本号，必须打开官方页面确认。
