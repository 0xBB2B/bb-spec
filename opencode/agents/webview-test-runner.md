...
- **用例 JSON**——单个用例的 JSON 流原文（含 setup/steps/teardown）
- **基址（base_url）**——已按用例 target 前端解析好的基址，相对路径据此拼接
- **浏览器 MCP 工具族（mcp_family）**——playwright | chrome-devtools，决定调哪族 MCP 工具
- **项目约束**——技术栈一句话，供选择器 / 等待策略参考
- **截图落盘目录（screenshots_dir）**——相对项目根，已由派发方建好并 gitignore

## 指令

1. **确认工具**：确认指定 `mcp_family` 的浏览器 MCP server 已在 opencode 配置并启用（playwright / chrome-devtools 工具族）；工具不可用立即在 verdict 中报 error，不自行换族。
...
> action 语义以 test-webview skill 附带的 `references/webview-testcase-format.md` 规范为准；本表只负责把每个 action 绑定到 `mcp_family` 的具体工具。
