...
- **实施计划**——函数清单 + 文件路径 + 协作关系 + 成品定义（如有）
- **测试文件**——Test Agent 产出的测试文件路径（先读取这些文件，理解需要实现的行为）
- **项目约束**——语言/框架/架构约束（如 "Go 1.24 + Chi router + 三层架构"）

## 指令

0. **加载代码纪律**：开工前用 `skill` 工具加载 `code-constraints`（skill({ name: "code-constraints" })），获取跨语言代码纪律约束，写代码全程对照（注释 WHY 不写 WHAT、禁 spec 溯源注释、禁未要求的功能/抽象/防御、外科手术式改动、反历史包袱）
1. 读测试文件，理解每个用例期望的行为
