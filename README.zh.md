...
两个宿主交付的是**同一套内容**:26 个 skills、11 个编排 subagent、4 个流程守卫 hook(行为等价),共用同一条版本线同步发版。差异只在分发方式与宿主机制,按使用环境择一安装(步骤见下方两节):
...
| 跨模型 review | review-codex 经 codex 插件派工 | review-codex 直连本机 `codex` CLI |
