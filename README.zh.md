...
**它们如何衔接。** 这条流水线是一场接力，而每一次交接物都是**磁盘上的文档**、不是会话里的临时记忆——这正是它能断点续、能换个 AI 接手、能逐级追溯的根本：`/spec` 把模糊需求落成 `.bb-spec/docs/spec/` 下的 spec 文档（**做什么**）→ `/plan` 读这些 spec、产出 `.bb-spec/docs/plan/` 下的函数级计划（**怎么做**）→ `/exec` 拿 plan 走 Test→Impl→Review、产出测试 + 代码、进度写入 `PROGRESS.md` → `/review` 审最终 diff → `/git-push-pr` 对照 spec 自查后开 PR。
...
- **`/plan`** — spec → **自包含、函数级**的实施计划：每个文件只解决一个独立问题，详细到函数名与职责（但不写具体代码），任何 AI 清空上下文后仅凭该文件即可正确实现。回答**"怎么做"**。
