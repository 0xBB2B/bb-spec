...
/prd   需求头脑风暴 → PRD 文档（可选上游；PM / 需求方使用，由 bb-spec-product 单独提供）
  ┊
/init-spec  存量项目反向 spec 化（仅首次接入时跑一次；并发分区提取）
  │
  ▼
...
/git-push-pr  pre-review → 推送 → 开 PR
  ...
  ┌────────────────────────────────────┐
  │  /doc-update 文档体检（定期 / 按需）  │
  │  扫全仓漂移 → 默认改 spec/文档        │
  │  代码明显不合理则停下问 → 挂 /revise   │
  └────────────────────────────────────┘
```
...
- **`/init-spec`** — *反向* spec 化存量项目：阅读现有代码 + 文档，把**已在执行的隐式规范**提炼成 ≤100 行、一文一规则的 spec，落点与 `/spec` 完全一致，让后续环节直接接力。庞大项目按分区拆 subagent 并发提取。仅首次接入时跑一次。
...
- **`/doc-update`** — **全仓 spec / 文档 / 代码一致性维护**。扫 spec、README、CLAUDE.md、plan 与代码，分六类漂移定位——spec-stale / doc-stale / code-violation / spec-conflict / orphan-index / uncovered-rule。默认方向是**代码是事实、spec/文档追平代码**；仅当代码明显违背硬约束或存在不合理设计才停下用 `AskUserQuestion` 请用户裁决，确认后挂回 `/revise` 走 TDD，本 skill 不直接动实现。批改前一次性汇总待裁决项 + 默认动作、整体批准后才落盘，过程中 INDEX 同步维护。与 `/init-spec`（零→有）、`/revise`（单点定向）、`/review` 的 `review-doc-sync`（PR diff 只读）划清边界。
...
| **bb-spec-workflow** _(核心功能)_ | spec → plan → exec → test-webview / test-api → review → revise → git-push-pr、init 反向 spec、doc-update 全仓一致性维护 + 11 个 subagent | `/plugin install bb-spec-workflow@0xbb2b` |
