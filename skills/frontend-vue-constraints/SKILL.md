...
- **依赖版本选择**：所有新增 / 升级依赖时，版本号选择遵循 `dependency-version-policy` skill（先查官方最新版本，不凭记忆填写）。
- **TypeScript 函数注释**：遵循 JSDoc/TSDoc 风格，使用中文（详见全局编码风格）。
- **Git 工作流**：前端项目同样遵循 `git-workflow-discipline` skill（分支策略、PR 三段式等）。
...
> ⚠️ 实际依赖版本号必须按 `dependency-version-policy` skill 要求**先查官方最新版本**再写入，不要凭记忆。
