...
**轻量修复判断**：进入修复前，评估改动规模。同时满足以下全部条件时，可向用户提议跳过 3-Agent 隔离直接修复：

- 归因为 impl-defect（spec/plan 无需变动）
- 改动 ≤ 1 个文件、≤ 10 行
- 修复逻辑显而易见（如拼写错误、off-by-one、条件取反）

提议话术：`"这个修复只改 <文件>，约 <N> 行，是否跳过 3-Agent 隔离直接修？"`
- 用户同意 → 主 agent 直接 TDD 修复（仍须先补失败测试再改实现），跳过 3a/3b/3c 中的 Agent 派发
- 用户拒绝或未回应 → 走标准 3-Agent 流程
